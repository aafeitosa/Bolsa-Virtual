...
public class EntityManagerFacade implements EntityManagerExtended {
	...
	public void persist(Object entity) {
		...
		// FIXME: It's a temporary solution to show errors on log files. Must be
	...
	public <T> List<T> findWithNamedQuery(String namedQueryName, Class<T> type) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Named, namedQueryName, type);
		return query.getList();
	}

	@Override
	public <T> List<T> findWithNamedQuery(String namedQueryName,
			Map<?, ?> parameters, Class<T> type) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Named, namedQueryName, type);
	...
	@Override
	public <T> PagedResult<T> findPageWithNamedQuery(String namedQueryName,
			Class<T> type, Map<?, ?> parameters, Integer start, Integer limit) {
		
		System.out.println(parameters);
		
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Named, namedQueryName, type, start, limit, parameters);
	 ...
	 */
	@Override
	public <T> List<T> findWithNamedNativeQuery(String namedNativeQueryName,
			Map<?, ?> parameters, Class<T> type) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Native, namedNativeQueryName, type);
		query.setParameters(parameters);
		return query.getList();
	}

	@Override
	public <T> PagedResult<T> findPageWithNativeQuery(String sql,
			Class<T> type, Integer start, Integer limit) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Native, sql, type, start, limit, null);
		return query.getPagedResult();
	}

	@Override
	public <T> PagedResult<T> findPageWithNativeQuery(String sql,
			String resultSetMapping, Map<?, ?> parameters, Integer start,
			Integer limit) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Native, sql, null, start, limit, parameters);
	...
	public <T> List<T> findWithNativeQuery(String sql, Class<T> type) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Native, sql, type, null, null, null);
		return query.getList();
	}
	...
	public <T> List<T> findWithQuery(String sql, Class<T> type) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Custom, sql, type);
		return query.getList();
	}

	@Override
	public <T> List<T> findWithQuery(String sql, Map<?, ?> parameters,
			Class<T> type) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Custom, sql, type);
		query.setParameters(parameters);
	...
	@Override
	public <T> PagedResult<T> findPageWithQuery(String sql,
			Map<?, ?> parameters, Class<T> type, Integer start, Integer limit) {
		QueryExecutor<T> query = new QueryExecutor<T>(entityManager,
				QueryType.Custom, sql, type, start, limit, parameters);
		return query.getPagedResult();
	}
